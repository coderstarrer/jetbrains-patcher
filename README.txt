
MANUAL INSTRUCTIONS(FOLLOW THIS AFTER RUNNING the crack batch file)



Click on the Options menu button on the bottom left corner. Click on Edit Custom VM Options... and then add the following in the dialogue box that opens:


-javaagent:/path/to/jetbra/ja-netfilter.jar=jetbrains
--add-opens=java.base/jdk.internal.org.objectweb.asm=ALL-UNNAMED
--add-opens=java.base/jdk.internal.org.objectweb.asm.tree=ALL-UNNAMED
Be sure to replace the path to ja-netfliter with the actual path.





The last two lines of what we pasted are needed to work on Java 17 and above, as per instructions from ja-netfilter.

If you have Java 17 and below you do NOT need these:
--add-opens=java.base/jdk.internal.org.objectweb.asm=ALL-UNNAMED
--add-opens=java.base/jdk.internal.org.objectweb.asm.tree=ALL-UNNAMED

So all you need to paste is: -javaagent:/path/to/jetbra/ja-netfilter.jar=jetbrains

Close and restart the program.


Go to the IDE, and click on the Options icon again on the bottom left hand corner. Click on Manage Licenses... and click Remove License. Confirm and click Remove License again on the pop-up.
Click on the Activation code radio button, and then clear everything in the text box, replacing it with the key we copied from the jetbra.in link.
That's it, ensure that the activation code shows an expiry date in the future. It won't expire, this is just how the program works.


KEY:

FV8EM46DQYC5AW9---


Will Work Forever!

